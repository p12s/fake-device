...
func TestDataReader(t *testing.T) {
	...
	for testNumber, test := range tests {
		...
		reader := telnet.NewDataReader(subReader)

		buffer := make([]byte, 2*len(test.Bytes))
		n, err := reader.Read(buffer)
		if nil != err && io.EOF != err {
			t.Errorf("For test #%d, did not expected an error, but actually got one: (%T) %v; for %q -> %q.",
				testNumber, err, err, string(test.Bytes), string(test.Expected))
			continue
		}

		if expected, actual := len(test.Expected), n; expected != actual {
			t.Errorf("For test #%d, expected %d, but actually got %d (and %q); for %q -> %q.",
				testNumber, expected, actual, string(buffer[:n]), string(test.Bytes), string(test.Expected))
			continue
		}

		if expected, actual := string(test.Expected), string(buffer[:n]); expected != actual {
			t.Errorf("For test #%d, expected %q, but actually got %q; for %q -> %q.",
				testNumber, expected, actual, string(test.Bytes), string(test.Expected))
			continue
		}
	}
}
